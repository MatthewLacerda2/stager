List of things to NOT do unless explicitly asked
  ...
  - Do NOT 'git commit'
  - NEVER use pragmas to silence warnings or errors
  - NEVER run migration generation or upgrades yourself, just tell me the commands instead.
...
There are `auxpy.py` and `auxmd.md`. They are scratchpads with throwaway code and notes. They only useful in short-term, hence why they are in the .gitignore. Do not edit these two files unless told to so.
